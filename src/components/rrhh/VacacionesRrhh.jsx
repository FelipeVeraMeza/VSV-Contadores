...
const RegistrarModal = ({ trabajador, empresaId, onClose }) => {
    ...
    return (
        <Dialog open onOpenChange={(v) => !v && onClose()}>
            <DialogContent className="sm:max-w-md bg-black/60 backdrop-blur-xl border-white/20 text-white">
                <DialogHeader><DialogTitle>Registrar vacaciones</DialogTitle></DialogHeader>
                <p className="text-sm text-gray-400 -mt-2">{trabajador.empleado} · saldo actual {dias(trabajador.saldo)}</p>
        ...
        <div className="space-y-5">
            <div className="relative max-w-md">
                ...
                <input value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar trabajador…"
                    className="w-full bg-white/[0.04] border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/50" />
            </div>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-white/10 bg-white/[0.03]">
                                ...
                                <th className="px-5 py-3.5 text-right text-[10px] font-semibold uppercase tracking-wider text-gray-500">Devengado</th>
                                <th className="px-5 py-3.5 text-right text-[10px] font-semibold uppercase tracking-wider text-gray-500">Tomadas</th>
                                <th className="px-5 py-3.5 text-right text-[10px] font-semibold uppercase tracking-wider text-gray-500">Saldo</th>
                        ...
                        <tbody className="divide-y divide-white/[0.06]">
                            {lista.length ? lista.map((f, i) => (
                                <motion.tr key={f.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25, delay: i * 0.03 }} className="hover:bg-white/[0.03]">
                                    <td className="px-5 py-3.5 text-sm text-white">{f.empleado}<div className="text-[10px] text-gray-500">{f.cargo}{f.zonaExtrema ? ' · zona extrema' : ''}</div></td>
                                    {consolidado && <td className="px-5 py-3.5 text-sm text-gray-400 truncate max-w-[200px]">{f.empresa || '—'}</td>}
                                    <td className="px-5 py-3.5 text-right text-sm text-gray-300">{dias(f.devengado)}</td>
                                    <td className="px-5 py-3.5 text-right text-sm text-gray-400">{dias(f.tomadas)}</td>
                                    <td className="px-5 py-3.5 text-right text-sm font-bold"><span className={f.saldo < 0 ? 'text-red-400' : 'text-emerald-400'}>{dias(f.saldo)}</span></td>
                                    <td className="px-5 py-3.5 text-right">
                                        <Button variant="ghost" size="sm" onClick={() => setReg(f)} className="h-8 text-cyan-400 hover:bg-cyan-500/10"><CalendarPlus className="h-4 w-4 mr-1.5" />Registrar</Button>
                                    </td>
                                </motion.tr>
                            )) : (
                                <tr><td colSpan={consolidado ? 6 : 5} className="text-center py-16">
                                    <div className="flex flex-col items-center text-gray-500">
                                        ...
                                        <h3 className="text-base font-semibold text-white">Sin trabajadores activos</h3>
