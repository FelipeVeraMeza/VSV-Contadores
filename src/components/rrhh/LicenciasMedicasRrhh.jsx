...
const TIPO_PILL = {
    comun: 'bg-blue-500/10 text-blue-400', maternal: 'bg-pink-500/10 text-pink-400',
    laboral: 'bg-amber-500/10 text-amber-400', prorroga: 'bg-violet-500/10 text-violet-400',
};
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center gap-3">
                ...
                {trabajadorId && <span className="text-xs text-gray-500">Registrando en <b className="text-gray-300">{periodoTexto(periodo)}</b></span>}
            </div>
            ...
            {trabajadorId && (
                <div className="rounded-2xl bg-white/[0.02] border border-white/10 p-4 space-y-3">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        <div>
                            <label className="text-[10px] font-semibold uppercase tracking-wider text-gray-500">Folio (opcional)</label>
                            <Input value={folio} onChange={e => setFolio(e.target.value)} placeholder="N° de licencia" className="bg-white/[0.04] border-white/10" />
                        </div>
                        <div>
                            <label className="text-[10px] font-semibold uppercase tracking-wider text-gray-500">Desde</label>
                            <Input type="date" value={fechaInicio} onChange={e => setFechaInicio(e.target.value)} className="bg-white/[0.04] border-white/10 [color-scheme:dark]" />
                        </div>
                        <div>
                            <label className="text-[10px] font-semibold uppercase tracking-wider text-gray-500">Hasta</label>
                            <Input type="date" value={fechaFin} onChange={e => setFechaFin(e.target.value)} className="bg-white/[0.04] border-white/10 [color-scheme:dark]" />
                        </div>
                    </div>
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-3">
                        <div className="w-full sm:w-32">
                            <label className="text-[10px] font-semibold uppercase tracking-wider text-gray-500">Días</label>
                            <Input type="number" min="1" max="31" value={dias} onChange={e => setDias(e.target.value)} placeholder="0" className="bg-white/[0.04] border-white/10" />
                        </div>
                        <div className="flex-1">
                            <label className="text-[10px] font-semibold uppercase tracking-wider text-gray-500">Observación (opcional)</label>
                            <Input value={glosa} onChange={e => setGlosa(e.target.value)} placeholder="Detalle…" className="bg-white/[0.04] border-white/10" />
                        </div>
                        <Button onClick={() => registrar.mutate()} disabled={registrar.isPending} className="bg-gradient-to-r from-purple-500 to-violet-600 hover:from-purple-600 hover:to-violet-700 text-white font-semibold h-10">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden">
                {isLoading ? (
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-white/[0.06]">
                                {lista.map((l, i) => (
                                    <motion.tr key={l.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2, delay: i * 0.02 }} className="group hover:bg-white/[0.03]">
                                        {!trabajadorId && <td className="px-5 py-3 text-white font-medium">{l.empleado}{l.empresa ? <span className="block text-[11px] text-gray-500">{l.empresa}</span> : null}</td>}
                                        <td className="px-5 py-3"><span className={`inline-flex px-2 py-0.5 text-[10px] font-semibold rounded-md ${TIPO_PILL[l.tipo] || 'bg-gray-500/10 text-gray-400'}`}>{TIPO_LABEL[l.tipo] || l.tipo}</span></td>
                                        <td className="px-5 py-3 text-gray-400">{periodoTexto(l.periodo)}</td>
                                        <td className="px-5 py-3 text-gray-400 whitespace-nowrap">{fmtFecha(l.fechaInicio)} – {fmtFecha(l.fechaFin)}</td>
                                        <td className="px-5 py-3 text-gray-400">{l.folio || '—'}</td>
                                        <td className="px-5 py-3 text-right"><span className="inline-flex items-center gap-1 font-semibold text-purple-300"><CalendarX className="h-3.5 w-3.5" />{l.dias}</span></td>
                                        <td className="px-5 py-3 text-right">
                                            <button onClick={() => quitar.mutate(l.id)} className="h-8 w-8 inline-flex items-center justify-center rounded-lg text-gray-400 hover:text-red-400 hover:bg-red-500/10 opacity-70 group-hover:opacity-100 transition"><Trash2 className="h-4 w-4" /></button>
                                        </td>
                                    </motion.tr>
                                ))}
                            </tbody>
                    ...
                    <div className="flex flex-col items-center text-gray-500 py-20">
                        <div className="w-14 h-14 rounded-2xl bg-white/[0.03] border border-white/10 flex items-center justify-center mb-4"><Stethoscope className="h-6 w-6 text-purple-400/60" /></div>
                        <h3 className="text-base font-semibold text-white">{trabajadorId ? 'Este trabajador no tiene licencias' : `Sin licencias en ${periodoTexto(periodo)}`}</h3>
