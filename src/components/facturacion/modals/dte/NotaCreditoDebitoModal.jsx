...
export default function NotaCreditoDebitoModal({ isOpen, setIsOpen, prefillData }) {
  ...
  const theme = {
    ...
    textClass: isCredito ? 'text-purple-600' : 'text-amber-600',
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(val) => !isSubmitting && setIsOpen(val)}>
      <DialogContent className="sm:max-w-[850px] w-[95vw] bg-white border-[#efe8dd] text-slate-700 p-0 shadow-2xl overflow-hidden rounded-2xl shadow-[0_0_80px_rgba(0,0,0,0.6)]">
        
        <div className="p-6 md:p-8 pb-4 border-b border-[#efe8dd] bg-white">
            <DialogHeader>
                <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
                    <div className={`p-4 rounded-2xl border bg-slate-50 ${theme.textClass} w-fit`}>
                        <ArrowDownUp size={28} />
                    </div>
                    <div>
                        ...
                        <DialogDescription className="text-slate-400 text-xs uppercase font-bold tracking-[0.2em] mt-1">Sincronizado con SII</DialogDescription>
                    </div>
                </div>
            </DialogHeader>
        </div>

        <div className="p-6 md:p-8 space-y-8 max-h-[75vh] overflow-y-auto custom-scrollbar bg-slate-50 w-full">
            
            <section className="space-y-3 w-full">
                <div className="flex items-center justify-between">
                    <Label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">1. Buscar Empresa o N° Factura</Label>
                    {clienteSel && (
                        <button onClick={() => {setClienteSeleccionado(null); setDocumentoOriginal(null); setSearchTerm("");}} className="text-[10px] font-black uppercase text-blue-500 hover:text-blue-600 transition-colors hover:underline">Limpiar Selección</button>
                    )}
                </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 w-full">
                        <div className="flex-[2] relative z-50">
                            <div className={`relative transition-all ${showSuggestions && filteredSuggestions.length > 0 && searchTerm ? 'ring-1 ring-blue-500 rounded-xl' : ''}`}>
                                ...
                                <Input 
                                    ...
                                    className={`pl-11 h-14 w-full bg-slate-50 border border-[#efe8dd] text-sm focus:border-emerald-500 transition-all uppercase ${showSuggestions && filteredSuggestions.length > 0 && searchTerm ? 'border-transparent rounded-t-xl rounded-b-none focus-visible:ring-0' : 'rounded-xl'}`} 
                                    disabled={!!prefillData?.doc || isSubmitting} 
                                />
                                {isLoadingDatos && <Loader2 className="absolute right-4 top-1/2 -translate-y-1/2 animate-spin text-slate-400" size={16} />}
                            </div>
                            
                            {showSuggestions && searchTerm && filteredSuggestions.length > 0 && (
                                <div className="absolute top-[100%] left-0 w-full bg-slate-50 border border-blue-500 border-t-0 rounded-b-xl shadow-2xl overflow-hidden flex flex-col animate-in fade-in slide-in-from-top-1">
                                    {filteredSuggestions.map((c, i) => (
                                        <button key={i} onMouseDown={() => selectCliente(c)} className="w-full px-5 py-4 text-left border-t border-[#efe8dd] hover:bg-slate-100 flex justify-between items-center group transition-colors">
                                            <div className="overflow-hidden pr-2">
                                                <p className="font-bold text-sm text-slate-700 group-hover:text-blue-600 truncate">{formatTextoVisual(c.nombre)}</p>
                                                <p className="text-[10px] font-mono text-slate-400 tracking-widest">{c.rut}</p>
                                            </div>
                        ...
                        <div className="flex-[1] relative z-40 min-w-[150px]">
                            ...
                            <Input 
                                ...
                                className="pl-11 h-14 w-full bg-slate-50 border border-[#efe8dd] rounded-xl text-sm focus:border-emerald-500 transition-all font-mono"
                                disabled={!!prefillData?.doc || isSubmitting} 
                            />
                        </div>
                    </div>
                ) : (
                    <div className="p-4 w-full bg-slate-50 border border-[#efe8dd] rounded-xl flex items-center gap-4 shadow-inner animate-in zoom-in-95 duration-200">
                        <div className={`p-3 rounded-lg bg-slate-50 ${theme.textClass}`}><User size={20}/></div>
                        <div className="overflow-hidden flex-1">
                            <p className="text-base font-black text-slate-900 truncate">{formatTextoVisual(clienteSel.nombre)}</p>
                            <p className="text-xs font-mono text-slate-400 tracking-widest">{clienteSel.rut}</p>
                        </div>
                    </div>
                )}
            </section>
            ...
            <section className="space-y-3 animate-in fade-in duration-300 w-full">
                ...
                <div className="border border-[#efe8dd] rounded-xl overflow-hidden bg-slate-50 shadow-sm w-full">
                    <div className="max-h-[250px] overflow-y-auto custom-scrollbar">
                        <table className="w-full text-left table-auto">
                            <thead className="sticky top-0 bg-slate-50 border-b border-[#efe8dd] z-10">
                                <tr>
                                    <th className="px-4 py-3 text-[10px] font-black uppercase text-slate-500 tracking-widest w-[40%]">Documento</th>
                                    <th className="px-4 py-3 text-[10px] font-black uppercase text-slate-500 tracking-widest w-[40%]">Cliente</th>
                                    <th className="px-4 py-3 text-[10px] font-black uppercase text-slate-500 tracking-widest text-right w-[20%]">Total</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-white/5">
                                {datosTabla.length === 0 ? (
                                    <tr><td colSpan={3} className="p-8 text-center text-xs font-bold text-slate-400 uppercase tracking-widest">No se encontraron documentos</td></tr>
                                ) : (
                                    datosTabla.map(doc => (
                                        <tr 
                                            key={doc.id} 
                                            onClick={() => handleSelectDocumentFromTable(doc)} 
                                            className={`cursor-pointer transition-colors ${documentoOriginal?.id === doc.id ? 'bg-slate-50' : 'hover:bg-slate-100'}`}
                                        >
                                            <td className="p-4 align-top">
                                                <div className="flex gap-4 items-start">
                                                    <div className="pt-0.5 flex-shrink-0">
                                                        {documentoOriginal?.id === doc.id ? <CheckCircle2 size={18} className={`${theme.textClass}`}/> : <div className="w-4 h-4 rounded-full border border-[#efe8dd]"/>}
                                                    </div>
                                                    <div className="overflow-hidden">
                                                        <span className={`text-xs font-black ${documentoOriginal?.id === doc.id ? theme.textClass : 'text-slate-600'} uppercase tracking-wider`}>#{doc.folio}</span>
                                                        <p className="text-[11px] font-black uppercase text-slate-700 mt-1 truncate">{NOMBRES_DTE[doc.tipo_dte] || `DTE ${doc.tipo_dte}`}</p>
                                                        <p className="text-[10px] font-mono text-slate-400 mt-0.5">{new Date(doc.fecha_emision).toLocaleDateString('es-CL')}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="p-4 align-top">
                                                <div className="pt-1">
                                                    <p className="text-xs font-bold text-slate-700 truncate max-w-[150px] md:max-w-[200px]">{formatTextoVisual(doc.razon_social)}</p>
                                                    <p className="text-[10px] font-mono text-slate-400 tracking-widest mt-1">{formatRutEstricto(doc.rut_cliente)}</p>
                                                </div>
                                            </td>
            ...
            {documentoOriginal && (
                <section className="space-y-6 animate-in slide-in-from-bottom-4 duration-500 pt-6 border-t border-[#efe8dd] w-full">
                    ...
                    <div className="grid grid-cols-2 gap-4 w-full">
                        <Button 
                            ...
                            className={`h-16 w-full border rounded-xl font-black uppercase text-[11px] tracking-widest flex items-center justify-center px-4 gap-2 transition-all ${tipoNota === '61' ? 'border-purple-500 bg-purple-500/10 text-purple-600 shadow-[0_0_15px_rgba(168,85,247,0.1)]' : 'border-[#efe8dd] text-slate-400 hover:bg-slate-100'} ${documentoOriginal.tipo_dte === 61 ? 'opacity-30 cursor-not-allowed disabled:bg-slate-50 disabled:border-[#efe8dd]' : ''}`}
                        >
                        ...
                        <Button 
                            ...
                            disabled={documentoOriginal.tipo_dte !== 61 || isSubmitting}
                            className={`h-16 w-full border rounded-xl font-black uppercase text-[11px] tracking-widest flex items-center justify-center px-4 gap-2 transition-all ${tipoNota === '56' ? 'border-amber-500 bg-amber-500/10 text-amber-600 shadow-[0_0_15px_rgba(245,158,11,0.1)]' : 'border-[#efe8dd] text-slate-400 hover:bg-slate-100'} ${documentoOriginal.tipo_dte !== 61 ? 'opacity-30 cursor-not-allowed disabled:bg-slate-50 disabled:border-[#efe8dd]' : ''}`}
                        >
                            <FileText size={18} className="hidden sm:block"/> Nota de Débito
                        </Button>
                    </div>

                    <div className="pt-4 flex flex-row gap-3 w-full border-t border-[#efe8dd]">
                        <Button variant="ghost" onClick={() => setIsOpen(false)} disabled={isSubmitting} className="flex-1 uppercase font-black text-[11px] tracking-widest h-14 rounded-xl text-slate-400 hover:text-slate-900 hover:bg-slate-100 transition-all border border-transparent hover:border-[#efe8dd]">Cancelar</Button>
                        <Button onClick={handleEmitir} disabled={isSubmitting || !documentoOriginal} className={`flex-[2] text-slate-900 font-black uppercase text-[11px] tracking-widest h-14 rounded-xl shadow-lg transition-all duration-300 ${isCredito ? 'bg-purple-600 hover:bg-purple-500 shadow-purple-500/20' : 'bg-amber-600 hover:bg-amber-500 shadow-amber-500/20'} disabled:opacity-50`}>
