...
import { 
  Receipt, Mail, CreditCard, Loader2, Tag, 
...
const DOC_CONFIG = {
  ...
  code: "DTE 34",
  description: "Emisión de servicios sin IVA sincronizada con el SII y registro en el Búnker.",
};
...
export default function ExentaElectronicaModal({ isOpen, setIsOpen }) {
  ...
  const onSelectCliente = (cliente) => {
    ...
    setItem(prev => ({
      ...
      descripcionProducto: "Servicios Contables Exentos"
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        tipo_documento: 34, // ⬅️ ¡MAGIA AQUI! Le decimos al robot que es EXENTA (DTE 34)
          ...
          <div className="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-zinc-950/95 backdrop-blur-md rounded-lg">
            {isSubmitting ? (
              <div className="flex flex-col items-center gap-4">
                ...
                <h3 className="text-xl font-bold uppercase italic tracking-tighter text-emerald-400">Emitiendo Factura Exenta...</h3>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-6 text-center">
                <CheckCircle2 className="h-20 w-20 text-emerald-500" />
                <h3 className="text-3xl font-black uppercase italic tracking-tighter">¡Exenta Emitida!</h3>
                <p className="font-mono text-lg text-gray-400 italic">Folio SII N° {folioGenerado}</p>
                <Button onClick={() => setIsOpen(false)} className="bg-emerald-600 hover:bg-emerald-700 w-full mt-4 rounded-xl font-black uppercase tracking-widest">Finalizar</Button>
              </div>
            )}
          </div>
        )}
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="mb-6 relative">
                  <div className="relative">
                    ...
                    <Input 
                      ...
                      className="pl-12 h-12 bg-black/40 border-white/10 rounded-2xl text-md focus:border-emerald-500 shadow-xl transition-all"
                    />
                  </div>
                  ...
                  {showSuggestions && (
                    <div className="absolute top-full left-0 w-full mt-2 bg-zinc-800 border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-[60] animate-in fade-in slide-in-from-top-2">
                      {filteredSuggestions.map((c, i) => (
                        <div key={i} onClick={() => onSelectCliente(c)} className="px-5 py-4 cursor-pointer hover:bg-emerald-600/20 border-b border-white/5 flex justify-between items-center group transition-colors">
                          <div>
                ...
                <div>
                  ...
                  {isExternal ? (
                    <Input 
                      ...
                      className="h-8 bg-emerald-500/10 border-emerald-500/30 text-xs rounded-xl focus:border-emerald-500 text-emerald-300 placeholder:text-emerald-500/40"
                    />
                  ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Plan / Concepto Exento</Label>
                <div className="relative">
                  <Tag className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" size={16} />
                  <Input value={item.name} onChange={(e) => setItem({...item, name: e.target.value})} className="pl-11 h-12 bg-black/40 border-white/10 rounded-xl" placeholder="Ej: Honorarios Profesionales" />
                </div>
              </div>
              <div className="space-y-2">
                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Valor Exento ($)</Label>
                <div className="relative">
            ...
            <div className="flex gap-3 pt-4 border-t border-white/5">
              ...
              <Button type="submit" disabled={!empresaEfectiva || isSubmitting} className="flex-[2] bg-emerald-600 hover:bg-emerald-500 text-white font-black uppercase text-[10px] tracking-[0.2em] h-12 rounded-xl shadow-lg shadow-emerald-500/20 transition-all">
                {isSubmitting ? 'Procesando Exenta...' : 'Emitir Factura Exenta SII'}
              </Button>
            </div>
          </form>
        </div>
