...
const NOMBRE_ARCHIVO = 'OFICINA VIRTUAL POR VENCER.csv';
...
//   RAZON SOCIAL | CORREO | FECHA INICIO | FECHA FIN | DIAS POR VENCER | ESTADO
...
// =====================================================================
// 📧 FUNCIÓN PARA ENVIAR EL CORREO DE AVISO (OFICINA VIRTUAL POR VENCER)
// =====================================================================
async function enviarCorreoPorVencer(cliente) {
    try {
        ...
        const asunto = `AVISO RENOVACIÓN OFICINA VIRTUAL - ${cliente.razonSocial}`;
        ...
        <div style="font-family: Arial, sans-serif; color: #333; line-height: 1.6; max-width: 800px;">
            ...
            <p>Espero te encuentres muy bien, escribo para informarte que la oficina virtual de tu empresa está <strong>próxima a vencer</strong>.</p>
            ...
            <p>
                ...
                Días para el vencimiento: <strong>${cliente.diasPorVencer}</strong>
            </p>

            <p>Te invitamos a renovar con anticipación para mantener tu domicilio comercial sin interrupciones. <strong>EN CASO DE NO RENOVAR FAVOR INFORMAR.</strong></p>
...
async function iniciarProceso() {
    console.log("==================================================");
    console.log("🚀 ENVÍO MASIVO: AVISO OFICINA VIRTUAL POR VENCER");
    console.log("==================================================\n");
    ...
    fs.createReadStream(RUTA_EXACTA)
        ...
        .on('data', (fila) => {
            ...
            if (razonSocial !== '') {
                clientes.push({
                    ...
                    fechaFin: buscarDato(fila, ['FECHA FIN', 'FIN CONTRATO', 'FECHA DE FIN', 'FIN'], 'Sin Fecha'),
                    diasPorVencer: buscarDato(fila, ['DIAS POR VENCER', 'DIAS PARA VENCER', 'DIAS RESTANTES', 'DIAS'], '0'),
        ...
        .on('end', async () => {
            ...
            for (let i = 0; i < clientesNotificar.length; i++) {
                ...
                await enviarCorreoPorVencer(clientesNotificar[i]);
            ...
            console.log("\n==================================================");
            console.log("🎉 ENVÍO MASIVO DE AVISO POR VENCER FINALIZADO");
            console.log("==================================================");
        })
        .on('error', (error) => {
